...
public class CaptureAndParsingHarFileForPixPngTPTest extends ReportGenerateThePoopleTest {
...
public void RecordHAR() throws InterruptedException {
	...
	proxy.enableHarCaptureTypes(CaptureType.RESPONSE_HEADERS);
		...
		driver1.get(
				"https://www.thepopple.com/");
		
		
		driver1.findElement(By.cssSelector("#promoted_stories_img_0")).click();
	...
	if (driver1 instanceof JavascriptExecutor) {
		...
		try {
			proxy.newHar("thepopple");	
driver1.get(newurl);
			
		

//		File screenshotFile = ((TakesScreenshot) driver1).getScreenshotAs(OutputType.FILE);
//		FileUtils.copyFile(screenshotFile, new File("/home/wittyfeed/ChromeDriver/SoftwareTestingMaterial.png"));
		...
		File harFile = new File("/home/wittyfeed/harfiles/thepopple.har");
		har.writeTo(harFile);
		driver1.quit();
	...
	public void ReadHarFile() {
		...
		driver.findElement(By.cssSelector("#fileSelector")).sendKeys("/home/wittyfeed/harfiles/thepopple.har");
	...
	public void Checkwriter_idValue() {
		...
		String writer_id = driver
				.findElement(By.cssSelector("#infoTabContent > dl:nth-child(2) > dd:nth-child(6) > ul:nth-child(1) > li:nth-child(7)"))
				.getText();
		try {
